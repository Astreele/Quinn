...
const roll: Command = {
  ...
  async execute(ctx) {
    ...
    const notationInput = ctx.parseString("notation", 0, true)?.trim() ?? null;
    ...
    const resolveRange = () => {
      if (!notationInput && firstNumber === null && secondNumber === null) {
      ...
      if (!notationInput && firstNumber === null && secondNumber !== null) {
        return {
          error:
            "Provide both minimum and maximum, or only a single maximum value.",
        };
      }
      ...
      if (!notationInput && firstNumber !== null && secondNumber !== null) {
        if (secondNumber < firstNumber) {
          return {
            error:
              "The maximum roll must be greater than or equal to the minimum.",
          };
        }
    ...
    for (const token of tokens) {
      ...
      if (unsignedToken.includes("d")) {
        ...
        if (!Number.isInteger(count) || count < 1) {
          await sendError("Each dice term must roll at least one die.");
          return;
        }
        ...
        if (count > 100) {
          await sendError("A single dice term cannot roll more than 100 dice.");
          return;
        }

        if (sides > 1000) {
          await sendError("A single die cannot have more than 1000 sides.");
          return;
        }
        ...
        total += modifier;
        rollDetails.push(`${modifier >= 0 ? "+" : ""}${modifier} modifier`);
      }
    }
