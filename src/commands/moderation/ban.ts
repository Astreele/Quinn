...
const ban: GuildCommand = {
  ...
  async execute(ctx) {
    ...
    if (!targetUser) {
      await ctx.reply(
        "Could not identify the user to ban. Please specify a valid user."
      );
      return;
    }
    ...
    try {
      ...
      if (member && !member.bannable) {
        await ctx.reply(
          "I do not have permission to ban this user. Check my roles and permissions."
        );
        return;
      }
      ...
      await targetUser
        .send(
          `You have been banned from **${ctx.guild.name}**. Reason: ${reason}`
        )
        .catch(() => null);
      ...
      await ctx.reply(
        `Successfully banned **${userDisplay}**. Reason: ${reason}`
      );
    } catch (error) {
      console.error(error);
