...
const clear: GuildCommand = {
  name: "clear",
  description:
    "Deletes a specified number of consecutive messages in a channel.",
  category: "moderation",
  conf: {
    modOnly: true,
    guildOnly: true,
  ...
  async execute(ctx) {
    ...
    try {
      ...
      const deleteCount =
        ctx instanceof MessageContext ? Math.min(amount + 1, 100) : amount;
      ...
      if (ctx instanceof CommandContext) {
        ...
        const msg = await channel.send(
          `Successfully deleted ${deleted.size - 1} message(s).`
        );
        setTimeout(() => {
      ...
      if (ctx instanceof CommandContext) {
        ...
        if (error?.code === 10008) {
          await ctx.command
            .followUp({
              content:
                "An error occurred. Messages older than 14 days cannot be bulk deleted.",
              ephemeral: true,
            })
            .catch(() => null);
        } else {
