...
const kick: GuildCommand = {
  ...
  async execute(ctx) {
    ...
    if (!targetMember.kickable) {
      await ctx.reply(
        "I do not have permission to kick this user. Check my roles and permissions."
      );
      return;
    }
    ...
    try {
      ...
      await targetMember
        .send(
          `You have been kicked from **${ctx.guild.name}**. Reason: ${reason}`
        )
        .catch(() => null);

      await targetMember.kick(reason);
      await ctx.reply(
        `Successfully kicked **${targetMember.user.tag}**. Reason: ${reason}`
      );
    } catch (error) {
      console.error(error);
