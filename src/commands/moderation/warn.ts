...
const warn: GuildCommand = {
  ...
  async execute(ctx) {
    ...
    try {
      ...
      await targetMember
        .send(`You have been warned in **${ctx.guild.name}** for: ${reason}`)
        .catch(() => null);
      await ctx.reply(
        `Successfully warned **${targetMember.user.tag}**. This user now has ${userWarnings.length} warning(s).`
      );
    } catch (error) {
      console.error(error);
      await ctx.reply(
        `Warning recorded for **${targetMember.user.tag}**, but I could not DM them.`
      );
    }
  },
};
